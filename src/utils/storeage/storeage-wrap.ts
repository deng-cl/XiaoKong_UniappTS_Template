...
class HStorage {
    ...
    set<T = any>(key: string, value: T, ttl?: number): void {
        try {
            ...
            uni.setStorageSync(namespacedKey, serialized);
        } catch (error) {
    ...
    get<T = any>(key: string): T | null {
        try {
            const namespacedKey = this.getNamespacedKey(key);
            let serialized = uni.getStorageSync(namespacedKey);
    ...
    remove(key: string): void {
        const namespacedKey = this.getNamespacedKey(key);
        uni.removeStorageSync(namespacedKey);
    }
    ...
    clearNamespace(): void {
        ...
        keys.filter((key) => key.startsWith(this.namespace)).forEach((key) => uni.removeStorageSync(key));
    }
}
